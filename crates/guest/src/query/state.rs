...
impl<D: QueryData, F: QueryFilter> QueryState<D, F> {
    ...
    pub fn from_raw(query_ptr: *mut (), data_state: D::State) -> Self {
        Self {
            query_ptr,
            state: data_state,
            _marker: PhantomData,
    ...
    pub fn iter_mut<'w, 's>(&'s mut self, world: &'w mut World) -> QueryIter<'w, 's, D, F> {
        ...
        let success =
            unsafe { bevy_query_state_iter_mut(world.ptr, self.query_ptr, &mut iter_ptr) };

        if !success || iter_ptr.is_null() {
            panic!("Failed to create query iterator");
        }

        QueryIter::new(iter_ptr, &mut self.state, world)
    }
}

impl<D: QueryData, F: QueryFilter> Drop for QueryState<D, F> {
    fn drop(&mut self) {
        if !self.query_ptr.is_null() {
            unsafe { bevy_query_state_drop(self.query_ptr) };
        }
    }
}
